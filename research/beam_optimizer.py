...
class BeamOptimizer(BaseOptimizer):
    ...
    def optimize(self):
        ...
        topk_states = self.vocab.itos[init_idxs][:, np.newaxis]
        for _ in range(self.max_steps):
            ...
            finished_beams_idx = (word_idxs == self.EOS).nonzero()
            n_finished_now = len(finished_beams_idx)
            if n_finished_now > 0:
                finished_beams += topk_states[finished_beams_idx.view(-1)].reshape(n_finished_now, -1).tolist()
                finished_beams_probs = np.concatenate((finished_beams_probs, topk_probs[finished_beams_idx].view(-1).detach().numpy()))
                not_finished_beams_idx = (word_idxs != self.EOS).nonzero().view(-1)
