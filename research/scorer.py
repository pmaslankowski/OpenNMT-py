...
class Scorer(object):
    ...
    def score_tokenized_texts(self, english_tok_seq_gen, german_tok_seq_gen, relaxed=False, method='max', normalize=False):
        if relaxed:
        ...
        return self.translator.score_target(src_data_iter=[' '.join(english_tok_seq) for english_tok_seq in english_tok_seq_gen],
                                            ...
                                            batch_size=consts.OPT.batch_size,
                                            relaxed=relaxed, method=method, normalize=normalize)
